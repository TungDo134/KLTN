...
from src.schemas import DayPlan, Place, ScheduledPlace, TripPlan, TripRequest
    ...
    def schedule(
        ...
        for day_idx in range(request.days):
            ...
            for place in day_places:
                ...
                if prev_id:
                    travel_mins = int(
                        round(travel_matrix.get(prev_id, {}).get(place.place_id, 0.0))
                    )
        ...
        return TripPlan(
            trip_request=request, days=day_plans, total_places=total_places_count
        )

    # Chia danh sách đã tối ưu vào số ngày yêu cầu
